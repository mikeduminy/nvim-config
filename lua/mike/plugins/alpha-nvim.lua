...
local quotes = require 'mike.utils.quotes'
local tableUtils = require 'mike.utils.table'
...
local buttons = {
  type = 'group',
  val = {
    button('s', '   Restore', strcmd 'SessionManager load_last_session'),
    button('r', '   Recents', strcmd 'Telescope oldfiles'),
    button('p', '   Projects', strcmd 'Telescope projects'),
    button('f', '   Search', strcmd 'Telescope find_files'),
    button('e', '   Create', strcmd 'ene <BAR> startinsert'),
    button('u', '   Update', strcmd 'PackerSync'),
    button('c', '   Config', strcmd '!tmux-sessionizer'),
    button('b', '   test', ":echo 'test'"),
    button('q', '   Quit', strcmd 'qa!'),
  },
  opts = {
    position = 'center',
    spacing = 1,
